function Lbl = segmentNucleiAndCytoplasmsECG(MD,well,varargin)
...
arg.cyto_threshmethod='otsu'; %minerr
...
elseif isa(arg.register,'Registration')
    ...
    [~,Tforms] = registerStack(stk,'reference',arg.reg_ref_frame);
    Reg.Tforms = Tforms;
...
for i=1:numel(NucLabels)
    ...
    % initialize the cytoplasm PxlIdx so that it has the same number of
    % elements
    PxlIdx_cyto = cell(size(PxlIdx_nuc));
    % remove nuclei from cytoplasm
    cytolbl(imerode(nuclbl>0,strel('disk',1)))=0;
    cytoRegProps = regionprops(cytolbl,'PixelIdxList');
    cytoRegPropsIxdList = {cytoRegProps.PixelIdxList};
    PxlIdx_cyto(1:length(cytoRegPropsIxdList))= cytoRegPropsIxdList;
...
%     PxlIdx_cyto = {PxlIdx_cyto.PixelIdxList};
    ...
    % initialize ix
    ix = zeros(size(PxlIdx_nuc));
    % combine all criteral to one true/false index & remove cells
    ix(1:length(Area)) = Area>arg.cyto_minarea  & Area<arg.cyto_maxarea  & ...
