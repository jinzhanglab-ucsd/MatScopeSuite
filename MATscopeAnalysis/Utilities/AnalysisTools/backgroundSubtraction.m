...
switch arg.samplingmethod
    ...
    case 'block'
        ...
        [c,r]=meshgrid(c,r);
    case 'roi'
        % Make the blksml be the size of the mask
        cols = find(sum(arg.msk)>0);
        rows = find(sum(arg.msk,2)>0);
        
        bcksml = zeros(length(rows),length(cols));
        bcksml = I(rows,cols);
    otherwise
...
% d=distance([x y]',[xi yi]'); % Replace with pdist2
d=pdist2([x y],[xi yi]);
[~,mi]=min(d);
x=x(mi);
y=y(mi);
...
%         bcksml(~msksml)=mean(I(arg.msk>0));
        % Just make the whole thing a flat number;
        bcksml(:,:)=mean(I(arg.msk>0));
    case 'griddata'
        % fit a
