...
% %            end
%            
%            %cd(fullfile(filesep,absPathParent))
%            
%            save(fullfile(absPathParent,pth,'Results.mat'),'R','Conclusions', '-v7.3')
%            fid=fopen(fullfile(absPathParent,pth,'Conclusions.txt'),'w'); 
%            fprintf(fid,'%s',R.Conclusions);  
%            fclose(fid); 
%            save(fullfile(absPathParent,pth,'Conclusions.mat'),'Conclusions', '-v7.3')
%            fid=fopen(fullfile(absPathParent,pth,'Classtype.txt'),'w'); 
%            fprintf(fid,'%s',class(R)); 
%            fclose(fid); 
%            
%         end
        ...
        function publish(R,varargin)
            ...
            if arg.redo 
                mydoc = publish(R.reportScript,'outputDir',fullfile(R.pth,R.reportPth));
        ...
        %% Having issues with run(R.analysisScript) if mmcorej is imported
        function runAnalysis(R)
            if ~isempty(R.analysisScript)
                
                if ispc
                    analysisPth = regexprep(R.analysisScript,'/',filesep);
                else
                    analysisPth = regexprep(R.analysisScript,'\',filesep);
                end
                
                [fileDir,script,ext] = fileparts(analysisPth);
                
                dirParDir = getAbsPath(fileDir);
                if isempty(dirParDir)
                    if ~exist(script,'file')
                        error(message('MATLAB:run:FileNotFound',script));
                    else
                        evalin('base',script);
                    end
                else
                    fileDir = fullfile(dirParDir,fileDir);
                    if ~isempty(fileDir)
