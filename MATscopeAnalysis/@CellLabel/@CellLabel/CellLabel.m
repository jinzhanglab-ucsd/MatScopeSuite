classdef CellLabel < handle
    ...
    properties
        ...
        ManualRoi; % Manual cell ROI verticies
        BkgrManualRoi; % Manual background ROI verticies
        BkgrManualLbl = zeros(0,0,'uint16');
        %%%
    end
    ...
    properties (Transient = true)
        ...
        ManualLbl = zeros(0,0,'uint16'); % Primarily used for live update
    ...
    methods
        ...
        function dumpToDrive(Lbl,pth,posname)
            ...
            B=Lbl.Bnd; 
            OL=Lbl.OldLbl; 
            save(Lbl.filename,'L','M','OL','B','Lbl', '-v7.3'); 
        ...
        %%%% ECG Edit - this seems to be causing an issue with having multiple ones open
...
%                     m = meanIntensityOverTime(s,lbl,arg.func);
                    ...
                    % Much much faster way of getting the mean values
                    ...
                    for k = 1:Nc
                        if ischar(arg.func)
                            switch arg.func
                                case 'mean'
                                    m(1,k)= nanmean(s(lbl==k));
%                                     for i=1:numel(cl)
%                                         avg{i}=nanmean(cl{i},1)';
%                                     end
                                case 'median'
                                    m(1,k)= nanmedian(s(lbl==k));
%                                     for i=1:numel(cl)
%                                         avg{i}=nanmedian(cl{i},1)';
%                                     end
                            end
                        else
                            m(1,k)= arg.func(s(lbl==k));
%                             for i=1:numel(cl)
%                                 avg{i}=arg.func(cl{i});
%                             end
                        end
%                         m(1,k)= mean(s(lbl==k));
        ...
        function M = LiveUpdateFuncPerLabel(Lbl,Img,varargin)
            % method will calculate a function (mean by defualt) per id per
            % time. It will use the label matrix that is closest in time to
            % the image measured in. 
            arg.type = 'base'; 
...
%         end
        ...
        %%% ECG Edits
        function [Isub,bkgrVal] = bkgrSubtractManualRoi(Lbl,ImageSet,varargin)
            % Use the manually specified ROI to subtract background from
            % image. 
            
            % Check that the BkgrManualRoi is set
            if isempty(Lbl.BkgrManualRoi{1,2})
                error('You must first specify the background ROI');
            else
                roiXY= Lbl.BkgrManualRoi{1,2}{1};
            end
            % Convert the background ROI to a mask
            msk = poly2mask(roiXY(:,1),roiXY(:,2),Lbl.sz(1),Lbl.sz(2));
            
            if ~isempty(varargin)
                % append the varargin with the speicific msk and settings
                % See if the smoothing method has been changed, if so leave it
                if max(strcmp('smoothmethod',varargin))==1
                    appendIn = [varargin,{'msk',msk,'samplingmethod','roi'}];
                else
                    appendIn = [varargin,{'msk',msk,'samplingmethod','roi','smoothmethod','none'}];
                end
            else
                appendIn ={'msk',msk,'samplingmethod','roi','smoothmethod','none'};
            end
            
            [Isub,bkg,~] = backgroundSubtraction(ImageSet,appendIn{:});
            
            bkgrVal = permute(mean(mean(bkg,2),1),[3 2 1]);
            
        end
        
        function setManualRoiGroup(Lbl,RoiCell,varargin)
            % add manually drawn ROI
            
            % Parse optional variables
            arg.frame_ind = 0; % if shifting ROI this will indicate when to start the shift
            arg.roi_type = 'Cells'; % change to 'Bkgr' for setting background ROI
            arg.roi_action = 'Add'; % 'Shift' when moving a whole set, 'Add' when adding a new one
                                      % and 'Del' when removing an ROI
            
            arg = parseVarargin(varargin,arg); 
            
            % Check that the RoiCell is a cell var
            if strcmp(arg.roi_action,'Del')
                if ~isnumeric(RoiCell)
                    error('For deleting ROI, RoiCell must be an integer number')
                elseif length(RoiCell)>1
                    error('For deleting ROI, RoiCell must be a single integer number')
                elseif RoiCell<0 || mod(RoiCell,1)~=0
                    error('For deleting ROI, RoiCell must be a single integer number')
                end
                del_ind = RoiCell;
            else
                if ~iscell(RoiCell)
                    error('Input ROI must be a cell vector of 2 collumn matricies');
                end
            end
            
            % Set ROI variable based on roi_type
            if strcmp(arg.roi_type,'Cells') ==1
                roi_prop = 'ManualRoi';
                
                % check for backwards compatability
                if ~isempty(Lbl.ManualRoi)
                    if size(Lbl.ManualRoi,2)~=3
                        nShifts = size(Lbl.ManualRoi,1);
                        baseNroi = length(Lbl.ManualRoi{1,2});
                        roiLeg = [1:baseNroi]';
                        for k = 1:nShifts
                            Lbl.ManualRoi{k,3}=roiLeg;
                        end
                    end
                end
            elseif strcmp(arg.roi_type,'Bkgr') ==1
                roi_prop = 'BkgrManualRoi';
            else 
                error('''roi_type'' must be either ''Cells'' or ''Bkgr'' ');
            end
            
            % Number of incoming ROI
            incomingNroi = length(RoiCell);
            
            % If frame_ind is not zero then the ROIs are shifting so there
            % needs to be the same number of ROI and there needs to be an
            % original roi set.
            switch arg.roi_action
                case 'Shift'
                    if ~isempty(Lbl.(roi_prop){1,2}) && length(Lbl.(roi_prop){1,2})==length(RoiCell)
                    if arg.frame_ind~=0
                        % Check that the frame_ind is a valid frame index number
                        if ismember(arg.frame_ind,[1:size(Lbl.Lbl,3)])~=1
                            error('Invalid frame index')
                        end

%                         % if initial ROI has not been set, use the RoiCell as
%                         % initial as well
%                         if isempty(Lbl.(roi_prop))
%                             Lbl.(roi_prop)(1,1:2) = {0,RoiCell};
%                         end

                        % Check that the number of ROI are the same as the initial
                        baseNroi = length(Lbl.(roi_prop){1,2});
                        

                        if baseNroi ~=incomingNroi
                            error('Number of ROI for shifted frame must be the same as the base ROI')
                        end
                        

                        % Check if shift for that frame already exists
                        allFrameInds = cell2mat(Lbl.(roi_prop)(:,1));
                        matchingFrameInd = find(allFrameInds==arg.frame_ind);


                        % Add shifted ROI
                        if strcmp(arg.roi_type,'Cells') ==1
                            if isempty(matchingFrameInd)
                                Lbl.ManualRoi = [Lbl.ManualRoi;{arg.frame_ind, RoiCell,Lbl.ManualRoi{1,3}}];    
                            elseif length(matchingFrameInd)==1
                                Lbl.ManualRoi(matchingFrameInd,1:2)={arg.frame_ind, RoiCell};
                            else
                                error('Issue with multiple matching frame inds');
                            end
                        else
                            if isempty(matchingFrameInd)
                                Lbl.BkgrManualRoi = [Lbl.BkgrManualRoi;{arg.frame_ind, RoiCell}];    
                            elseif length(matchingFrameInd)==1
                                Lbl.BkgrManualRoi(matchingFrameInd,1:2)={arg.frame_ind, RoiCell};
                            else
                                error('Issue with multiple matching frame inds');
                            end
                        end
                        ...
                        % Overwrite any Roi that are saved as this is supposed to
                        % be the initialization ROI

                        nRoi = length(RoiCell);
                        Lbl.(roi_prop) = {0,RoiCell,[1:nRoi]'};

                    end
                    else
                        error('ROIs must already exist to be shifted');
                    end
                case 'Add'
                    if strcmp(roi_prop,'ManualRoi')
                        if isempty(Lbl.ManualRoi)
                            Lbl.ManualRoi = {0,RoiCell,[1:incomingNroi]'};
                        else
                            nShifts = size(Lbl.ManualRoi,1);
                            oldLeg = Lbl.ManualRoi{1,3};
                            if isempty(oldLeg)
                                newLeg = [1:incomingNroi]';
                            else
                                maxLeg = max(Lbl.ManualRoi{1,3});
                                newLeg = [oldLeg',maxLeg+1:maxLeg+incomingNroi]';
                            end
                            for m = 1:nShifts
                                oldRoiCell = Lbl.ManualRoi{m,2};
                                combCell = [oldRoiCell;RoiCell];
                                
                                Lbl.ManualRoi{m,2} = combCell;
                                Lbl.ManualRoi{m,3} = newLeg;
                            end
                        end
                    else
                        Lbl.BkgrManualRoi = {0,RoiCell};
                    end
                case 'Del'
                    if strcmp(roi_prop,'ManualRoi')
                        if isempty(Lbl.ManualRoi)
                            warning('No ROI to delete')
                        elseif del_ind == 0
                            error('Roi number to delete must be supplied')
                        else
                            nShifts = size(Lbl.ManualRoi,1);
                            delRow = find(Lbl.ManualRoi{1,3}==del_ind,1);
                            if isempty(delRow)
                                error('Roi number supplied does not exist')
                            else
                                for m = 1:nShifts
                                    oldRoiCell = Lbl.ManualRoi{m,2};
                                    oldRoiCell(delRow,:) = [];
                                    oldLeg = Lbl.ManualRoi{m,3};
                                    oldLeg(delRow,:) = [];
                                    Lbl.ManualRoi{m,2} = oldRoiCell;
                                    Lbl.ManualRoi{m,3} = oldLeg;
                                end
                            end
                        end
                    else
        ...
        function updateLblFromManual(Lbl,varargin)
            
            arg.sx = 2048;
            arg.sy = 2048;
            
             if Lbl.sz(1)~=0 && Lbl.sz(2)~=0
                arg.sx=Lbl.sz(1);
                arg.sy=Lbl.sz(2);
             end
            ...
            % Get all Lbl time points
            allT = Lbl.T;
            
            sortT = sort(allT);
            nT = length(sortT);
            
            nROIgroups = size(Lbl.ManualRoi,1);
            
            if nROIgroups == 0
                % If there are no ROI specified do nothing
                warning('No ROI are specified. CellLabels not changed');
                
            else
                if nROIgroups == 1
                    % Just one ROI set so change all Lbl to that ROI
                    
                    sortFrameMap = [1,nT];
                    roiVertsCell = Lbl.ManualRoi{1,2};
                    groupLbl{1} = Lbl.genLblFromROI(roiVertsCell,arg.sx,arg.sy);
                    
                    
                else
                    
                    allFrameInds = cell2mat(Lbl.ManualRoi(:,1));
                    [sortInds,indMap] = sort(allFrameInds);
                    sortFrameMap = [];
                    groupLbl = {};
                    for k = 1:nROIgroups
                        
                        if k==1
                            sortFrameMap(k,1:2) = [1,sortInds(k+1)-1];
                        elseif k == nROIgroups
                            sortFrameMap(k,1:2) = [sortInds(k),nT];
                        else
                            sortFrameMap(k,1:2) = [sortInds(k),sortInds(k+1)-1];
                        end
                        
                        roiVertsCell = Lbl.ManualRoi{indMap(k),2};
                        groupLbl{k} = Lbl.genLblFromROI(roiVertsCell,arg.sx,arg.sy);
                    end
                ...
                % Update the Lbls
                for k = 1:nROIgroups                    
                    for m = sortFrameMap(k,1):sortFrameMap(k,2)
                        Lbl.addLbl(groupLbl{k},'base',sortT(m)) % Currenly only set up for having as base
                    end
                end
            end
        ...
        function lblMat = genLblFromROI(Lbl,roiVertsCell,sz_x,sz_y,varargin)
            ...
            if strcmp(arg.roi_type,'Cells') ==1
                % Initialize matrix
                lblMat = zeros(sz_x,sz_y,'uint16');
                
                % number of ROI
                nROI = size(roiVertsCell,1);
                
                % create masks
                for k = 1:nROI
                    roiXY = roiVertsCell{k};
                    mask = poly2mask(roiXY(:,1),roiXY(:,2),sz_x,sz_y);
                    lblMat(mask~=0) = mask(mask~=0)*k;
                end
            elseif strcmp(arg.roi_type,'Bkgr') ==1
                roiXY = roiVertsCell{1};
                lblMat = poly2mask(roiXY(:,1),roiXY(:,2),sz_x,sz_y);
            else
                error('''roi_type'' must be either ''Cells'' or ''Bkgr'' ');
            end
        ...
        function genLUmanualLbl(Lbl,sz_x,sz_y,varargin)
            ...
            elseif strcmp(arg.roi_type,'Bkgr') ==1
                roi_prop = 'BkgrManualRoi';
                lbl_prop = 'BkgrManualLbl';
            else 
                error('''roi_type'' must be either ''Cells'' or ''Bkgr'' ');
            end
            
            if ~isempty(Lbl.(roi_prop))
                roiVertsCell = Lbl.(roi_prop){1,2};
                
                Lbl.(lbl_prop) = Lbl.genLblFromROI(roiVertsCell,sz_x,sz_y,'roi_type',arg.roi_type);
            else
                warning('The manual ROIs have not been set, no Lbl was generated');
            end
            
        end

        function roiVals = getLUmeanIntensityPerCell(Lbl,imgIn,varargin)
            ...
            if strcmp(arg.roi_type,'Cells') ==1
                % Number of cells;
                Nc = max(max(Lbl.ManualLbl));
                roiVals  = zeros(1,Nc);
                for k = 1:Nc
                    roiVals(1,k) = mean(imgIn(Lbl.ManualLbl==k));
                end
            elseif strcmp(arg.roi_type,'Bkgr') ==1
                roiVals = mean(imgIn(Lbl.BkgrManualLbl));
            else 
                error('''roi_type'' must be either ''Cells'' or ''Bkgr'' ');
            end
        end
