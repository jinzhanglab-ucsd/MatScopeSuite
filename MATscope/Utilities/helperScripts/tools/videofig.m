function [fig_handle, axes_handle, scroll_bar_handles, scroll_func,plot_axes_handle] = ...
	videofig(num_frames, redraw_func, play_fps, big_scroll, ...
	key_func, num_of_extra_axes, varargin)
...
%   João Filipe Henriques, 2010
	...
	%initialize figure
    ix = find(ismember(varargin(1:2:end),'fig'),1); 
    v=varargin; 
    if ~isempty(ix)
        fig_handle=varargin{2*ix}; 
        varargin([2*ix-1 2*ix])=[]; 
    elseif isempty(varargin{2*ix})
        fig_handle=figure;
        varargin([2*ix-1 2*ix])=[];
    else
        fig_handle=figure; 
    ...
    switch num_of_extra_axes
        case 0
    
            %main drawing axes for video display
            axes_handle = axes('Position',[0 0.03 1 0.97],'DrawMode','fast');
        case 1
            axes_handle = axes('Position',[0 0.03 0.5 0.97],'DrawMode','fast');
            plot_axes_handle = axes('Position',[0.55 0.05 0.5 0.92],'DrawMode','fast');
        case 2
            axes_handle = axes('Position',[0 0.03 0.5 0.97],'DrawMode','fast');
            plot_axes_handle(1) = axes('Position',[0.55 0.05 0.5 0.4],'DrawMode','fast');
            plot_axes_handle(2) = axes('Position',[0.55 0.45 0.5 0.4],'DrawMode','fast');
        case 3
            axes_handle = axes('Position',[0 0.03 0.5 0.97],'DrawMode','fast');
            plot_axes_handle(1) = axes('Position',[0.55 0.05 0.5 0.3],'DrawMode','fast');
            plot_axes_handle(2) = axes('Position',[0.55 0.35 0.5 0.3],'DrawMode','fast');
            plot_axes_handle(3) = axes('Position',[0.55 0.65 0.5 0.3],'DrawMode','fast');
    end
	
    % create the zoomed version axes
    h_plot = axes('Position',[0 0.03 0.1 0.1],'drawmode','fast','visible','off','tag','plot_axes');  %#ok<NASGU>
    ...
    % adjust fps if needed based on time it takes to draw
%     tic
%     redraw_func(f)
%     draw_time=toc; 
%     if 1/play_fps < draw_time
%         play_fps=1./(draw_time);
%         play_fps=round(play_fps*100)/10; % max resolution of msec to prevent warning message
%         warning('Slowing refrash time to %g since drawing each frame is slow',draw_time)
%     end
       
    function on_delete(src,event) %#ok, unused arguments
        stop(play_timer);
        delete(play_timer); 
    end
