function communicatingSubmitFcn(cluster, job, props, ...
    clusterHost, remoteJobStorageLocation,account,username,keyfile,time,queue,ppn)
...
remoteConnection = getRemoteConnection(cluster, clusterHost, remoteJobStorageLocation,username,keyfile);
...
if regexp(clusterHost, 'gordon')
additionalSubmitArgs = sprintf('-V -v QOS=0,Catalina_license_request_list=matlab_%d -l nodes=%d:ppn=%d:native,walltime=%s -q %s -A %s',numberOfNodes*procsPerNode,numberOfNodes, procsPerNode,time,queue,account);
elseif regexp(clusterHost, 'trestles')
additionalSubmitArgs = sprintf('-V -v QOS=0,Catalina_license_request_list=matlab_%d -l nodes=%d:ppn=%d,walltime=%s -q %s -A %s',numberOfNodes*procsPerNode,numberOfNodes, procsPerNode,time,queue,account);
elseif regexp(clusterHost, 'tscc')
additionalSubmitArgs = sprintf('-V -l nodes=%d:ppn=%d,walltime=%s -q %s -A %s',numberOfNodes, procsPerNode,time,queue,account);
end
