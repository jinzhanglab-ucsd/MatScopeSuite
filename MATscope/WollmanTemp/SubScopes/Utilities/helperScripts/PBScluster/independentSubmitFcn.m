function independentSubmitFcn(cluster, job, props, ...
    clusterHost, remoteJobStorageLocation,account,username,keyfile,time,queue)
...
for ii = 1:numberOfTasks
    ...
    additionalSubmitArgs = sprintf('-V -v QOS=0,Catalina_license_request_list=matlab_1 -l nodes=1:ppn=1:native,walltime=%s -q %s -A %s',time,queue,account);
    elseif regexp(clusterHost, 'trestles')
    additionalSubmitArgs = sprintf('-V -v QOS=0,Catalina_license_request_list=matlab_1 -l nodes=1:ppn=1,walltime=%s -q %s -A %s',time,queue,account);
    elseif regexp(clusterHost, 'tscc')
    additionalSubmitArgs = sprintf('-V -l nodes=1:ppn=1,walltime=%s -q %s -A %s',time,queue,account);
    end
