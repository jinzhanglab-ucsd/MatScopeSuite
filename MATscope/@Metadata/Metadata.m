classdef Metadata < handle
    ...
    properties
        ...
        defaultTypes = {   'Zindex' 'Z' 'Channel'    'Exposure'    'Fluorophore'    'Marker'    'PixelSize'    'PlateType'    'Position'    'Skip'    'TimestampFrame'    'TimestampImage'    'XY'    'acq'    'frame'    'group' 'AllInputs'}' 
...
%             pth=regexprep(pth,'data3','bigstore'); 
            ...
            %%% ECG edit %%% trying to make more flexable about file
            %%% location
            
%             [absPathParent,pth] = getAbsPath(pth);
            
            % if a Metadata.mat file exist just load it
            if exist(fullfile(pth,'Metadata.mat'),'file')
                if exist(fullfile(pth,'Metadata.dsv'),'file')
                    MD=readDSV(MD, pth); 
                else
...
%             end
        ...
        function new_md = readDSV(MD, pth)
           %TODO read Metadata from deliminator separated file
%            if nargin==1
%                fname = 'Metadata.dsv';
%            end
           delimiter = '\t';
           s = load(fullfile(pth, 'Metadata.mat'));
           dsv = csvimport(fullfile(pth, 'Metadata.dsv'), 'delimiter', delimiter);
           types = dsv(1, 2:end);
           values = dsv(2:end, :);
           s.MD.Values = values(:, 2:end);
           s.MD.Types = types;
           s.MD.ImgFiles = values(:, 1)';
           new_md = s.MD;
        end
...
%                 msgbox('Warning - added image already exist in Metadata object!'); 
        ...
        function markEvent(MD,desc,conc,units,type,location)
            % Add event to Events table
            
            %Current number of events
            [nEvents,~] = size(MD.Events);
            
            % Add the event to the list
            MD.Events(nEvents+1,1:6) = {now,desc,conc,units,type,location};
...
%                     end
        ...
        function saveMetadata(MD,pth)  
            ...
            V=MD.Values;
%             vcount = size(V, 1);
%             oldvcount = size(MD.OldValues, 1);
%             newvalues = V{end-(vcount-oldvcount):end, :};
            MD.appendMetadataDSV(pth, 'Metadata.dsv');
            MD.Values={};
        ...
        function appendMetadataDSV(MD, pth, fname)
            ...
            if vcount-oldvcount > 0
                ...
                if exist(fullfile(pth, fname))
                    ...
                    md_export_csv = [newfnames newvalues];
                    cell2csvAppend(fullfile(pth, fname), md_export_csv, delimiter);
                    MD.OldValuesSize = size(MD.Values, 1);
                else
%                     f = fopen(fullfile(pth, fname), 'wt');
%                     fprintf(f, ['Username:' MD.Username '\nProject:' MD.Project '\nDataset:' MD.Dataset '\nacqname' MD.acqname '\n']);
%                     fclose(f);
                    cell2csv(fullfile(pth, fname), cat(2, {'filename'}, MD.Types), delimiter);
        ...
        function exportMetadata(MD, pth, varargin)
            ...
            cell2csv(fullfile(pth, 'Metadata.csv'), md_export_csv); 
        end
